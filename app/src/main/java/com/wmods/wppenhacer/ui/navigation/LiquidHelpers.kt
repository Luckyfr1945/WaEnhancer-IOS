 ...
 * Adapted for WaEnhancer Floating Liquid Navigation Bar
 */
...
// --- 1. DAMPED DRAG ANIMATION (Fisika Pegas / Spring Physics) ---
...
// --- 2. INTERACTIVE HIGHLIGHT (Efek Kilau Sentuhan / Shine) ---
class InteractiveHighlight(
    ...
    val modifier: Modifier = Modifier.drawWithContent {
        ...
        if (progress > 0f) {
            ...
            val glowRadius = (size.minDimension * 1.5f).coerceAtLeast(1f)
            drawRect(
...
// --- 3. DRAG INSPECTOR (Helper Gesture) ---
suspend fun PointerInputScope.inspectDragGestures(
    ...
    awaitEachGesture {
        ...
        onDragStart(down)
        onDrag(initialDown, Offset.Zero) // initial
        val upEvent = drag(pointerId = initialDown.id) {
...
private suspend inline fun AwaitPointerEventScope.awaitDragOrUp(pointerId: PointerId): PointerInputChange? {
    ...
    while (true) {
        ...
        } else if (dragEvent.previousPosition != dragEvent.position) return dragEvent
    }
}
